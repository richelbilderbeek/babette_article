...
\verb;babette; can be installed easily from CRAN \textbf{NOTE: This is not true yet: 
we are still in the process of submitting to CRAN}:
...
\end{lstlisting}
Instead of using all taxa, any subset of taxa can be selected 
to have a dated common ancestor.
...
\end{lstlisting}
This file can then be loaded and edited by BEAUti, run by BEAST2, or run by \verb;babette;: 
...
These output files can then be inspected with other tools, or used
to continue a BEAST2 run. \verb;bbt_run; supports specifying the folder and name of these files,
which defaults to a temporary folder to keep
the working directory clean of intermediate and
temporary files. 
